// Vendored from BenaliHQ/workdesk-terminal @ 297fea0a (MIT, internetvin/Vin Verma).
// See ./NOTICE.md for fork provenance and the rename audit.
//
// Single-file rebrand for use as the WorkdeskOS terminal layer. The only
// modifications from upstream are:
//   1. `VIEW_TYPE` value changed from `"vin-terminal-view"` to
//      `"workdesk-terminal"` so it slots into our existing constants.
//   2. The upstream `export default class TerminalPlugin extends Plugin`
//      shell is stripped — our `src/main.ts` owns the Plugin lifecycle and
//      registers this view itself. Helper-script init is exported as
//      `writePtyHelper(pluginDir)` so our `onload` can call it.
//   3. `VIEW_TYPE`, `TerminalView`, `ShortcutsModal`, `OutputCaptureModal`
//      added to the public exports.
// All other behavior, DOM classes (`.vin-terminal-*`), and CSS are unchanged
// from upstream so we track future vin releases with minimal merge friction.

import { ItemView, WorkspaceLeaf, App, TFile, setIcon, SuggestModal, Modal, Menu } from "obsidian";
import { Terminal } from "@xterm/xterm";
import { FitAddon } from "@xterm/addon-fit";
import type { ChildProcess } from "child_process";

export const VIEW_TYPE = "workdesk-terminal";
let ptyHelperPath = "";
...
export class OutputCaptureModal extends SuggestModal<CaptureOption> {
...
// --- WorkdeskOS integration export ---
//
// Replaces the upstream `TerminalPlugin.onload` helper-script init step.
// Our plugin shell (src/main.ts) calls this once during onload, before
// `registerView(VIEW_TYPE, ...)` runs, so the first session can spawn its
// PTY against a written-out helper file.
//
// BRAT and Obsidian's plugin installer only ship main.js + manifest.json +
// styles.css; the python helper has to be materialized at runtime.
export function writePtyHelper(vaultBasePath: string, manifestDir: string): void {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const fs = require("fs") as typeof import("fs");
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const path = require("path") as typeof import("path");
  const helperPath = path.join(vaultBasePath, manifestDir, "pty-helper.py");
